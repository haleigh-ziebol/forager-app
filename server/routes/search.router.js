...
//GET search specifying all terms
router.get('/', (req, res) => {
  ...
  if(req.isAuthenticated() && region !== '%') {
    let queryText = `SELECT DISTINCT on (s.id) r.name, s.*, o.user_id FROM species s
    ...
    let queryText = `SELECT DISTINCT on (s.id) s.*, o.user_id 
                    FROM species s
                    FULL JOIN observations o
                    ON s.id = o.species_id
